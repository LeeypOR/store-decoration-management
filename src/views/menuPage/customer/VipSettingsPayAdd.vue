...
<template>
  <div>
    ...
    <a-form :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item >
        <a-radio-group v-model="RCData.receiveWay">
          <a-radio :value="1">
            付费购买
          </a-radio>
          <a-radio :value="2">
   ...
   <!--   <div style="font-weight: bold;margin: 20px 0;font-size: 18px;">
      ...
      <a-form-item label="权益">
        ...
        <a-checkbox @change="integralRatioChange">
        </a-checkbox>
       ...
       <a-input-number :min="1"  v-model="RCData.integralRatio"  style="width: 120px;margin-left: 15px;"
        ...
        <a-checkbox @change="couponChange">
        </a-checkbox>
        ...
        <br>
        <a-checkbox @change="sendGoodsChange">
        </a-checkbox>
...
<!--      <div style="font-weight: bold;margin: 20px 0;font-size: 18px;">
      ...
      <a-form-item label="分享设置">
        <a-checkbox @change="shareChange">
          允许分享
        </a-checkbox>
      </a-form-item>

      <a-form-item label="卡券码兑换权益卡">
        <a-checkbox @change="exchangeCard" >
          允许兑换
        </a-checkbox>
...
<script>
  ...
  export default {
    ...
    created() {
     this.getlastestvipinfo();
    },
    methods: {
      moment,

      getlastestvipinfo(){
        this.$get('goldenhome/member/admin_/member/card/benefits').then((res)=>{
          if (res && res.returnCode ==1000){
            console.log(res.dataInfo)
            this.RCData.benefitList = res.dataInfo
     ...
     /*   this.RCData.payRuleList.forEach((item) => {
        ...
        if (this.RCData.goodsList.length>0){
         ...
         let RCData = deepCopy(this.RCData)
          ...
          this.$post('goldenhome/member/admin_/member/card/add', {
            ...RCData
          }).then(res => {
            if (res && res.returnCode === 1000) {
              console.log('***********')
              console.log(res)
              this.$message.success('添加付费卡成功!')
              this.$router.push({
                name: 'CustomerVipSettings',
                query: {key:'2'}
              })
...
/*      shareChange(e){
      ...
      deleteRule(record){
        console.log(record)
        // this.RCData.payRuleList.length > 0
