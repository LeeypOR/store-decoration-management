...
<!--      <a-form-item label="使用须知">
     ...
     <!--   <a-col>
      ...
      <a-form-item label="分享设置">
        <a-checkbox disabled @change="shareChange">
          允许分享
        </a-checkbox>
      </a-form-item>
...
<script>
  ...
  export default {
    ...
    watch: {
     ...
     created() {
      //  /goods/query/norm
      /*this.$get('/goods/query/norm',{goodsId:"438441546660970496"}).then((res)=>{
        console.log(res)
      })*/
    },
    methods: {
    ...
    /*  cardName(){
      ...
      handleCancel(){
        this.$router.push({ name: 'CustomerRightCardRightCard'})
      },
      // 日期区间选择
   ...
   /*   useTimeAndTimesChange(){
     ...
     async chooseDisCountGoods(){
      ...
      couponChange(e){
        ...
        if (e.target.checked===false){
          // this.RCData.couponList.splice(0)
          // this.RCData.couponListkeys.splice(0)
       ...
       if (this.RCData.goodsList.length>0){
        ...
        // console.log('表单数据:',RCdata)
          if (RCData.background===0){
              RCData.bgImage = ''
              console.log('表单数据:',RCData)
            this.$post('goldenhome/member/admin_/benefits/card/add', {
            ...RCData
            }).then(res => {
              if (res && res.returnCode === 1000) {
                console.log('***********')
                console.log(res)
                this.$message.success('添加权益卡成功!')
                this.$router.push({
