    ...
    <a-form :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
     ...
     <!-- <a-form-item  >
      ...
      <a-form-item>
        ...
        <br>
        <a-checkbox @change="islowpriceChange" v-model="islowprice"></a-checkbox>
        <span style="line-height: 40px;"> 自定义</span>
        <a-button type="link"
                  :disabled="!islowprice"
                  @click="openModel">{{makePointData.chooseActPage}}</a-button>
        <span style="color: darkgray">注：需先在店铺装修配置自定义权益微页面</span>
        <br>
        <div style="margin-left: 50px;">
          <a-radio-group v-show="islowprice && makePointData.mspageId"
                         v-model="makePointData.issueAging2"
                         @change="issueAgingChange2">
            ...
            <a-date-picker
              :disabled="makePointData.issueAging2!=2"
              :show-time="{ defaultValue: Moment('00:00:00', 'HH:mm:ss') }"
              v-model="makePointData.receiveTimeAmount"
              format="YYYY-MM-DD"
              value-format="YYYY-MM-DD"
              style="width:200px" />
            <br>
            <br>
            <a-radio  style="margin-left: 105px;"  :value="3">
              指定时间范围:
            </a-radio>

            <a-date-picker
              :disabled="makePointData.issueAging2!=3"
              :show-time="{ defaultValue: Moment('00:00:00', 'HH:mm:ss') }"
              v-model="makePointData.startTime"
              format="YYYY-MM-DD "
              value-format="YYYY-MM-DD"
              style="width:200px" />
        ...
        <div>
          <a-checkbox @change="sendCouponChange" v-model="issendCoupon"></a-checkbox>
          发放优惠券
          <a-button type="link"
                    :disabled="!issendCoupon"
                    @click="chooseCoupon">选择优惠券></a-button>
          <span style="color: darkgray">注：此处优惠券将直接发放到用户的“我的优惠券”列表中</span>
          <br>
        <div v-if="couponList.length>0">
          <span style="margin-left: 27px;" >发放时间：</span>

          <a-date-picker
            :disabled="couponList.length==0"
            v-model="makePointData.couponIssueDate"
            format="YYYY-MM-DD"
            value-format="YYYY-MM-DD"
            style="width:200px" />
        ...
        <br>
        <div>
          <a-checkbox @change="backintegralRatioChange" v-model="isbackintegralRatio"></a-checkbox>
          <span style="line-height: 40px;"> 购物返积分倍率: </span>
          <a-input-number :min="1" :disabled="!isbackintegralRatio" v-model="makePointData.integralRatio"></a-input-number> 倍;
          <span style="color: darkgray;">注：设置用户生日当天购物返积分的倍数</span>
        </div>
      </a-form-item>
      <!--      <a-form-item >
              <template slot="label">
    ...
    methods: {
      ...
      async getInfo() {
        ...
        if (this.formData.id) {
          ...
          this.makePointData.couponIssueDate =  this.formData.couponIssueDate
          ...
          if (!!this.makePointData.mspageId){
            if (this.formData.issueAging==3){
              this.makePointData.issueAging2 = 2
              this.makePointData.receiveTimeAmount =  this.formData.startDate
            }else if(this.formData.issueAging==4){
              this.makePointData.issueAging2 = 3
              this.makePointData.startTime =  this.formData.startDate
              this.makePointData.endTime =  this.formData.endDate
            }
          }
          // this.makePointData.issueAging2 = this.formData.issueAging // 1  3  4
          // this.makePointData.validDuration = this.formData.validDuration //
      ...
      /*  if (this.makePointData.gradeIdList.length==0){
        ...
        console.log("integralRatio:",integralRatio)
        ...
        if (this.makePointData.issueAging2 == 1){
          issueAging = 1
        }else if(this.makePointData.issueAging2 == 2) {
          issueAging = 3
          startDate = this.makePointData.receiveTimeAmount
        }else if(this.makePointData.issueAging2 == 3){
          issueAging = 4
          startDate = this.makePointData.startTime
          endDate = this.makePointData.endTime
        }
        if (this.couponList.length>0 && this.makePointData.couponIssueDate==''){
          return this.$message.error('请选择优惠券发放时间')
        }
        ...
        if (this.formData.id) {
          ...
          this.$post('goldenhome/member/admin_/member/benefits/update', {
            ...
            startDate,
            endDate,
            couponIssueDate:this.makePointData.couponIssueDate,
            ...
            // validDuration:issueAging !="" ? this.makePointData.validDuration:null,
            integralRatio
          }).then(res => {
            if (res && res.returnCode === 1000) {
              console.log('***********')
    ...
    */
      ...
      /*    issueAgingChange(){
            if (this.makePointData.issueAging !=1){
              ...
              this.makePointData.validDuration = 0;
      ...
      issueAgingChange2(){
        if (this.makePointData.issueAging2 ==1){
          // 置空下时间
          this.makePointData.startTime = ""
          this.makePointData.endTime = ""
          this.makePointData. receiveTimeAmount = ""
        }
        ...
        if (this.makePointData.issueAging2 ==3){
            // 置空上面的时间
          this.makePointData. receiveTimeAmount = ""
        }
      },
      ...
      sendCouponChange(e){
        ...
        if (this.issendCoupon ==false){
          ...
          this.couponList = []
          this.couponListkeys = []
          this.makePointData.couponIssueDate = ''
        }
      },
