      ...
      >
        ...
        >
          <span :style="[textStyle, bgColor]"
            >减{{
              parseFloat(
                ((l.salePrice * 1 - l.discountPrice * 1) / 100).toFixed(2)
              )
            }}元</span
          >{{ l.goodsName }}
        </div>
        ...
        <div class="progress" v-show="formData.listStyle == 0">
          <a-progress
            class="progressBar"
            :percent="(l.totalRemainStock * 10 * 10) / l.totalDiscountStock"
            size="small"
            :show-info="false"
        ...
        <div class="listPrice">
          ...
          >
            ¥<span class="moneyNum" :style="textStyle">{{
              ((l.discountPrice * 1) / 100).toFixed(2) || 99
            }}</span
            >起
          </div>
          <div
        ...
        <div class="price">
          <div>
            ...
            ><span
              ...
              v-show="formData.seckillPrices && formData.listStyle == 0"
              :style="textStyle"
              >{{ ((l.discountPrice * 1) / 100).toFixed(2) || 99 }}</span
            >
            <span
    ...
    formData() {
      ...
      if (this.item.config.formData && this.initNum <= 1) {
        ...
        if (this.item.config.formData.shopList.length > 0) {
          ...
          const PromiseList = [];
          const a = this.$post(
            "goldenhome/seckill/admin_/discount/activity/list",
            ids
          );
          PromiseList.push(a);
          Promise.all(PromiseList).then((res) => {
            if (res[0].dataInfo != null) {
              ...
              this.item.config.formData.shopList.forEach((item) => {
                const p = this.$get(
                  "goldenhome/seckill/admin_/discount/goods/list",
                  {
                    activityId: item.id,
                  }
                );
