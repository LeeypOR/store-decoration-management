...
export default {
  ...
  methods: {
    ...
    async customRequest(file){
      console.log(this.accept,file,this.accept.includes(file.file.type),file.file)
      if(this.limit!==0 && this.fileList.length>=this.limit){
        this.$message.error(`最多上传${this.limit}个文件`)
        return;
      }
      let index=file.file.name.lastIndexOf('.');
      let ext=file.file.name.substr(index+1);
      if(this.accept && !this.accept.includes(ext)){
        this.$message.error('上传文件格式不在允许范围');
        return;
      }
      if(this.fileSizeLimit!==0 && file.file.size>this.fileSizeLimit){
        let fileSize=(this.fileSizeLimit / (1024 * 1024)).toFixed(1)+'M';
        console.log('fileSize',fileSize);
        this.$message.error(`文件大小超出${fileSize}限制！`)
        return;
      }
      this.loading=true;
      ...
      let res = await this.$post(`${BASE_URL}/upload`,form,{noAlertFlag:true});
